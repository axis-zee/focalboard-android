...
class AuthRepository(
    ...
    suspend fun login(serverUrl: String, email: String, password: String): Result<AuthResponseV2> {
        return try {
            ...
            // Note: The v2 API may return HTML instead of JSON for CSRF endpoint
            // In that case, we'll use a default token or extract from response
            val csrfToken = try {
            ...
            val response = api.login(csrfToken, LoginRequestV2(email, password))
